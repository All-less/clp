...
namespace compressor_frontend {
    ...
    public:
        ...
        void set_token (uint32_t pos, RDToken& value) {
            m_storage.set_value(pos, value);
        }

        [[nodiscard]] const RDToken&  get_token (uint32_t pos) const {
            return m_storage.get_value(pos);
        }

        void set_curr_token (RDToken& value) {
            if (m_token_callback != nullptr) {
                m_token_callback(value.get_string(), m_type);
            }
            m_storage.set_curr_value(value);
        }

        [[nodiscard]] const RDToken& get_curr_token () const {
        ...
        [[nodiscard]] Buffer<RDToken>& storage () {
            return m_storage;
        }
        ...
        void set_template(std::string tpl) {
            m_template = tpl;
        }
        ...
        void set_token_callback(std::function<void(std::string, size_t)> callback) {
    ...
    private:
        ...
        Buffer<RDToken> m_storage;
        std::vector<RDToken*> m_variable_occurrences;
        std::string m_template;
        std::string m_msg;
        std::function<void(std::string, size_t)> m_token_callback;
    };
}
