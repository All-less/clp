...
public:
    ...
    /**
     * Return the AST that relates this nonterminal's children together (based on the
     * production/syntax-rule that was determined to have generated them)
     * @return std::unique_ptr<ParserAST>
     */
    std::unique_ptr<ParserAST>& getParserAST() { return m_ast; }
...
public:
     ...
     * @return bool
     */
    [[nodiscard]] bool is_epsilon() const { return this->m_body.empty(); }
...
public:
    ...
    bool empty() const { return m_kernel.empty(); }
...
public:
     ...
     * @param rule
     */
    void add_rule(std::string const& name, std::unique_ptr<RegexAST<NFAStateType>> rule);
...
private:
     ...
     * @return ItemSet*
     */
    ItemSet* go_to(ItemSet*, uint32_t const&);
    ...
    bool symbol_is_token(uint32_t s) { return m_terminals.find(s) != m_terminals.end(); }
