...
using RegexASTByte = compressor_frontend::finite_automata::RegexAST<
        compressor_frontend::finite_automata::RegexNFAByteState>;
using RegexASTGroupByte = compressor_frontend::finite_automata::RegexASTGroup<
        compressor_frontend::finite_automata::RegexNFAByteState>;
using RegexASTIntegerByte = compressor_frontend::finite_automata::RegexASTInteger<
        compressor_frontend::finite_automata::RegexNFAByteState>;
using RegexASTLiteralByte = compressor_frontend::finite_automata::RegexASTLiteral<
        compressor_frontend::finite_automata::RegexNFAByteState>;
using RegexASTMultiplicationByte = compressor_frontend::finite_automata::RegexASTMultiplication<
        compressor_frontend::finite_automata::RegexNFAByteState>;
using RegexASTOrByte = compressor_frontend::finite_automata::RegexASTOr<
        compressor_frontend::finite_automata::RegexNFAByteState>;
using RegexASTCatByte = compressor_frontend::finite_automata::RegexASTCat<
        compressor_frontend::finite_automata::RegexNFAByteState>;
...
unique_ptr<SchemaFileAST> SchemaParser::generate_schema_ast(ReaderInterface& reader) {
    ...
    std::unique_ptr<SchemaFileAST> schema_file_ast(
            dynamic_cast<SchemaFileAST*>(nonterminal.getParserAST().release())
    );
...
static unique_ptr<ParserAST> regex_identity_rule(NonTerminal* m) {
    return unique_ptr<ParserAST>(new ParserValueRegex(
            std::move(m->nonterminal_cast(0)->getParserAST()->get<unique_ptr<RegexASTByte>>())
    ));
}
...
static unique_ptr<ParserAST> regex_middle_identity_rule(NonTerminal* m) {
    return unique_ptr<ParserAST>(new ParserValueRegex(
            std::move(m->nonterminal_cast(1)->getParserAST()->get<unique_ptr<RegexASTByte>>())
    ));
}
...
static unique_ptr<ParserAST> regex_literal_rule(NonTerminal* m) {
    ...
    return unique_ptr<ParserAST>(new ParserValueRegex(
            unique_ptr<RegexASTByte>(new RegexASTLiteralByte(token->get_string()[0]))
    ));
}
...
static unique_ptr<ParserAST> regex_existing_integer_rule(NonTerminal* m) {
    ...
    assert(token->get_string().size() == 1);
    return unique_ptr<ParserAST>(new ParserValueRegex(
            unique_ptr<RegexASTByte>(new RegexASTIntegerByte(r2_ptr, token->get_string()[0]))
    ));
}
...
static unique_ptr<ParserAST> regex_new_integer_rule(NonTerminal* m) {
    ...
    assert(token->get_string().size() == 1);
    return unique_ptr<ParserAST>(new ParserValueRegex(
            unique_ptr<RegexASTByte>(new RegexASTIntegerByte(token->get_string()[0]))
    ));
}
...
void SchemaParser::add_productions() {
    ...
    add_production(
            ...
            new_schema_file_rule_with_delimiters
    );
    add_production(
            "SchemaFile",
            {"SchemaFile", "PortableNewLine"},
            identity_rule_ParserASTSchemaFile
    );
    add_production(
            "SchemaFile",
            {"SchemaFile", "PortableNewLine", "Comment"},
            identity_rule_ParserASTSchemaFile
    );
    add_production(
            "SchemaFile",
            {"SchemaFile", "PortableNewLine", "SchemaVar"},
            std::bind(&SchemaParser::existing_schema_file_rule, this, std::placeholders::_1)
    );
    add_production(
            "SchemaFile",
            {"SchemaFile", "PortableNewLine", "Delimiters", "Colon", "DelimiterString"},
            existing_schema_file_rule_with_delimiter
    );
    add_production(
            "DelimiterString",
