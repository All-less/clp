...
public:
    ...
    void add_tag(int const& rule_name_id) { m_tags.push_back(rule_name_id); }

    [[nodiscard]] std::vector<int> const& get_tags() const { return m_tags; }

    bool is_accepting() { return !m_tags.empty(); }

    void add_byte_transition(uint8_t const& byte, RegexDFAState<stateType>* dest_state) {
...
public:
    ...
    DFAStateType* new_state(std::set<NFAStateType*> const& set);

    DFAStateType* get_root() { return m_states.at(0).get(); }
