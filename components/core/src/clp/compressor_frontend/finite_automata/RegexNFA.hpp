...
public:
    ...
    void set_accepting(bool accepting) { m_accepting = accepting; }

    [[nodiscard]] bool const& is_accepting() const { return m_accepting; }

    void set_tag(int rule_name_id) { m_tag = rule_name_id; }

    [[nodiscard]] int const& get_tag() const { return m_tag; }
    ...
    void clear_epsilon_transitions() { m_epsilon_transitions.clear(); }

    [[nodiscard]] std::vector<RegexNFAState<stateType>*> const& get_epsilon_transitions() const {
    ...
    void clear_byte_transitions(uint8_t byte) { m_bytes_transitions[byte].clear(); }

    [[nodiscard]] std::vector<RegexNFAState<stateType>*> const& get_byte_transitions(uint8_t byte
    ) const {
        return m_bytes_transitions[byte];
    }

    void reset_tree_transitions() { m_tree_transitions.reset(); }

    Tree const& get_tree_transitions() { return m_tree_transitions; }
