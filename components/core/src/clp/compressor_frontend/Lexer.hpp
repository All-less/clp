...
public:
    ...
    static inline const std::vector<int> cTokenEndTypes = {(int)SymbolID::TokenEndID};
    static inline const std::vector<int> cTokenUncaughtStringTypes
    ...
    Lexer()
            : m_byte_buf_pos(0),
              ...
              m_active_byte_buf(nullptr),
              m_byte_buf_ptr(nullptr),
              m_byte_buf_size_ptr(nullptr),
              m_static_byte_buf_ptr(nullptr) {
     ...
     * @param delimiters
     */
    void add_delimiters(std::vector<uint32_t> const& delimiters);
     ...
     * @param regex
     */
    void add_rule(uint32_t const& id, std::unique_ptr<RegexAST<NFAStateType>> regex);
     ...
     * @return RegexAST*
     */
    RegexAST<NFAStateType>* get_rule(uint32_t const& name);
     ...
     * @param value
     */
    void set_reduce_pos(uint32_t value) { m_reduce_pos = value; }

    [[nodiscard]] bool const& get_has_delimiters() const { return m_has_delimiters; }

    [[nodiscard]] bool const& is_delimiter(uint8_t byte) const { return m_is_delimiter[byte]; }

    // First character of any variable in the schema
    [[nodiscard]] bool const& is_first_char(uint8_t byte) const { return m_is_first_char[byte]; }
...
private:
    ...
    char** m_byte_buf_ptr;
    uint32_t const* m_byte_buf_size_ptr;
