...
void load_lexer_from_file(
    ...
    SchemaParser sp;
    unique_ptr<compressor_frontend::SchemaFileAST> schema_ast
            = sp.generate_schema_ast(schema_reader);
    ...
    lexer.add_rule(
            ...
            std::move(make_unique<RegexASTLiteral<finite_automata::RegexNFAByteState>>(
                    RegexASTLiteral<finite_automata::RegexNFAByteState>('\n')
            ))
    );
        ...
        if (contains_delimiter) {
            ...
            if (ErrorCode_Success != error_code) {
                throw std::runtime_error(
                        schema_file_path + ":" + to_string(rule->m_line_num + 1) + ": error: '"
