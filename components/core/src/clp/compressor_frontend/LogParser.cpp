...
void LogParser::add_rules(unique_ptr<SchemaFileAST> const& schema_ast) {
    // Currently, required to have delimiters (if schema_ast->delimiters != nullptr it is already
    // enforced that at least 1 delimiter is specified)
...
void LogParser::increment_uncompressed_msg_pos(ReaderInterface& reader) {
    ...
    if (m_uncompressed_msg_pos == m_uncompressed_msg_size) {
        ...
        if (m_active_uncompressed_msg == m_static_uncompressed_msg) {
            ...
            memcpy(m_active_uncompressed_msg,
                   m_static_uncompressed_msg,
                   sizeof(m_static_uncompressed_msg));
        }
        m_uncompressed_msg_size *= 2;
        m_active_uncompressed_msg = (Token*)
                realloc(m_active_uncompressed_msg, m_uncompressed_msg_size * sizeof(Token));
        if (m_active_uncompressed_msg == nullptr) {
            ...
            throw(err);  // error of this type will allow the program to continue running to
...
void LogParser::parse(ReaderInterface& reader) {
    ...
    while (true) {
        ...
        if (token_type == (int)SymbolID::TokenEndID) {
            m_archive_writer_ptr->write_msg_using_schema(
                    m_active_uncompressed_msg,
                    m_uncompressed_msg_pos,
                    m_lexer.get_has_delimiters(),
                    has_timestamp
            );
            break;
        }
        bool found_start_of_next_message
                = (has_timestamp && token_type == (int)SymbolID::TokenNewlineTimestampId)
                  || (!has_timestamp
                      && m_active_uncompressed_msg[m_uncompressed_msg_pos].get_char(0) == '\n'
        ...
        if (found_start_of_next_message) {
            increment_uncompressed_msg_pos(reader);
            m_active_uncompressed_msg[m_uncompressed_msg_pos]
                    = m_active_uncompressed_msg[m_uncompressed_msg_pos - 1];
            if (m_active_uncompressed_msg[m_uncompressed_msg_pos].m_start_pos
                == *m_active_uncompressed_msg[m_uncompressed_msg_pos].m_buffer_size_ptr - 1)
            {
            ...
            m_active_uncompressed_msg[m_uncompressed_msg_pos - 1].m_type_ids
                    = &Lexer<RegexNFAByteState, RegexDFAByteState>::cTokenUncaughtStringTypes;
            m_lexer.set_reduce_pos(
                    m_active_uncompressed_msg[m_uncompressed_msg_pos].m_start_pos - 1
            );
            m_archive_writer_ptr->write_msg_using_schema(
                    m_active_uncompressed_msg,
                    m_uncompressed_msg_pos,
