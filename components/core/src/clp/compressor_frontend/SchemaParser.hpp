...
public:
    ...
    void add_character(char character) { m_name.push_back(character); }
...
public:
    ...
    SchemaVarAST(
            std::string name,
            std::unique_ptr<RegexAST<RegexNFAByteState>> regex_ptr,
            uint32_t line_num
    )
...
public:
    ...
    void add_delimiter(uint32_t delimiter) { m_delimiters.push_back(delimiter); }
...
public:
     ...
     * @return std::unique_ptr<SchemaFileAST>
     */
    static std::unique_ptr<SchemaFileAST> try_schema_file(std::string const& schema_file_path);
