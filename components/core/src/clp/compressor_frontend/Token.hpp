...
public:
    ...
    Token(uint32_t start_pos,
          uint32_t end_pos,
          char** buffer_ptr,
          uint32_t const* buffer_size_ptr,
          uint32_t line,
          std::vector<int> const* type_ids)
            : m_start_pos(start_pos),
              m_end_pos(end_pos),
              m_buffer_ptr(buffer_ptr),
              m_buffer_size_ptr(buffer_size_ptr),
              m_line(line),
              m_type_ids(type_ids) {}
    ...
    /**
     * Return the first character (as a string) of the token string (which is a delimiter if
    ...
    uint32_t m_end_pos;
    char** m_buffer_ptr;
    uint32_t const* m_buffer_size_ptr;
    uint32_t m_line;
